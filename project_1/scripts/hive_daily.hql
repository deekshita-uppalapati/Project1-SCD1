...
load data inpath '/user/saif/project_1/day/' into table day;


insert into temp select d.custid, d.username, d.quote_count, d.ip, d.entry_time, d.prp_1, d.prp_2, d.prp_3, d.ms, d.http_type, d.purchase_category, d.total_count, d.purchase_sub_category, d.http_info, d.status_code, current_date(), d.year_col, d.month_col
...
insert overwrite table temp2 select custid, username, quote_count, ip, entry_time, prp_1, prp_2, prp_3, ms, http_type, purchase_category, total_count, purchase_sub_category, http_info, status_code, year_col, month_col from scd_tgt where date_col=current_date();
